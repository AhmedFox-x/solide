...
router.get("/qr/page", async (_req: Request, res: Response) => {
  try {
    ...
    res.send(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><meta http-equiv="refresh" content="5"><title>WhatsApp QR</title><style>body{font-family:sans-serif;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;background:#111;color:#fff;text-align:center}div{max-width:400px}img{width:300px;height:300px;border-radius:12px}p{color:#888;margin-top:20px}p.sub{color:#555;font-size:12px;margin-top:30px}</style></head><body><div><h2>📱 امسح QR برابط واتساب</h2><img src="data:image/png;base64,${qrCode}" alt="QR Code"/><p>افتح واتساب ← الأجهزة المرتبطة ← ربط جهاز</p><p class="sub">QR يتجدد تلقائياً كل 5 ثواني</p></div></body></html>`);
  } catch {
